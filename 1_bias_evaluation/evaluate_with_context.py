...
# Per-model default decision_question_id lists (from your @bash selections)
MODEL_DEFAULT_QIDS: Dict[str, List[int]] = {
    # Llama 8B
    "Meta-Llama-3-8B-Instruct": [40, 12, 94],
    # Qwen 4B
    "Qwen3-4B": [72, 62, 71],
}

# Hard-coded evaluation plan (no CLI args needed)
EVAL_JOBS = [
    {
        "model_path": "/mnt/nfs/huggingface/Qwen/Qwen3-4B/",
        "model_type": "auto",
        "target_qids": [72, 62, 71],
    },
    {
...
def plot_grouped_bar_chart(
    ...
    """Create grouped bar chart comparing different prompt conditions."""
    try:
        import matplotlib
        matplotlib.use("Agg")
    ...
    ax.set_xlabel('Scenario (Sorted by Original Bias)', fontsize=14, fontweight='bold')
    ax.set_ylabel('Mean |P(Yes|Group A) - P(Yes|Group B)|', fontsize=14, fontweight='bold')
    ax.set_title(f'Fairness Gaps Across Conditions with Context Prompts ({model_name})', 
                 fontsize=16, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels([f"Q{qid}\n{context_names.get(qid, '')}" for qid in qids], fontsize=11)
...
def run_one_job(job: dict) -> None:
    ...
    output_plot = os.path.join(output_dir, f"context_comparison_{model_name}.png")
    print("\nGenerating plot...")
