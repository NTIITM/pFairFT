...
# Python 脚本路径：在 discrim-eval 配对数据上做「随机负向干预」评估
...
# 统一输出的 CSV 文件路径（聚合所有模型的随机头干预结果）
CSV_PATH="per_sample_intervention_negative_random_all_models.csv"
...
for MODEL_DIR in "$LLM_RESEARCH_DIR"/* "$QWEN_DIR"/*; do
  ...
  echo "=========================================="
  echo "Running RANDOM NEGATIVE head-level intervention discrim-eval for model: $MODEL_NAME"
  ...
  echo "Finished random negative intervention discrim-eval for model: $MODEL_NAME"
  echo ""
done

echo "=========================================="
echo "All done. Per-sample random-negative-intervention details CSV: $CSV_PATH"
echo "=========================================="
