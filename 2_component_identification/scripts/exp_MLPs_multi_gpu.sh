...
# Python 脚本路径（MLP 敏感度分析）
PY_SCRIPT="/home/common1/hwluo/project/pFairFT/exp2/analyze_race_sensitive_MLPs.py"
...
for MODEL_DIR in "$LLM_RESEARCH_DIR"/* "$QWEN_DIR"/*; do
  ...
  if [ -f "$CSV_PATH" ]; then
    ...
    echo "Skip MLP analysis for this model."
  fi
done
...
run_model_on_gpu() {
  ...
  echo "[GPU $GPU_ID] Starting MLP race sensitivity analysis for model: $MODEL_NAME"
  ...
  if [ ! -f "$CSV_PATH" ]; then
    ...
    echo "[GPU $GPU_ID] Skip MLP analysis for this model."
    return 1
  fi
  
  # 为每个模型创建 MLP 分析输出目录
  MODEL_OUTPUT_DIR="${EXP2_DIR}/sensitive_MLPs_${MODEL_NAME}_top100"
  ...
  echo "[GPU $GPU_ID] Finished MLP sensitivity analysis for model: $MODEL_NAME"
...
echo "=========================================="
echo "All done. MLP race-sensitivity analysis completed for all models."
echo "=========================================="
