...
第三步：公平约束损失函数 (Fairness Loss)
  - 在训练时，除了要完成原本的任务（KL 散度约束），还要多加一个约束
...
最终损失函数：
  L = L_task + λ * L_f
  其中：
  - L_task: KL 散度损失（约束 fact/counterfactual 输出一致）
...
def train_epoch(
    ...
    for step, batch in enumerate(pbar):
        ...
        # 总损失：L = L_task + λ * L_f
        # loss = kl_loss + fairness_lambda * fairness_loss
        loss = fairness_loss
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Weight for fairness loss (λ in L = L_task + λ * L_f).",
    )
    parser.add_argument(
        "--bf16",
        action="store_true",
    ...
    print(f"Fairness lambda (λ): {args.fairness_lambda}")
    print("=" * 80)
